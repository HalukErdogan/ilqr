...
class RK3IntegrationStepper : public IntegrationStepper<N, M> {
   public:
    void Step(const SystemPtr& system, const State& state, const Control& control,
        ...
        system->CalculateDynamics(state + delta_time / 2.0 * k1, control, time + delta_time / 2.0, k2);
        system->CalculateDynamics(state - delta_time * k1 + 2.0 * delta_time * k2, control, time + delta_time, k3);
